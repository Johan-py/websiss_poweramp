...
import { Search, BookOpen, Clock, Users, CheckCircle2 } from "lucide-react";
...
export function Inscripciones() {
  ...
  useEffect(() => {
    Promise.all([api.ofertas.list(), api.carreras.list()]).then(([data]) => {
      setOfertas(
        data.map((o: any) => ({
          id: o.id,
  ...
  const filtered = ofertas.filter((o) =>
    search ? o.materiaNombre.toLowerCase().includes(search.toLowerCase()) : true,
  );

  return (
    <div className="flex h-[calc(100vh-6rem)] -m-6 lg:-m-8">
      <div className="w-64 shrink-0 border-r bg-card p-5 flex flex-col gap-4">
        <div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between p-4 border-b">
          <div>
            <h2 className="text-sm font-semibold">Oferta Académica</h2>
            <p className="text-xs text-muted-foreground">{filtered.length} materias disponibles</p>
          </div>
        </div>
      ...
      <div className="w-80 shrink-0 border-l bg-card p-5">
        {selected ? (
          <div className="space-y-5">
            ...
            <Button className="w-full h-10" disabled={selected.estado !== "abierta"}>
              {selected.estado === "abierta" ? <><CheckCircle2 className="h-4 w-4 mr-2" />Inscribirse</> : selected.estado === "completa" ? "Sin cupos disponibles" : "Oferta cerrada"}
            </Button>
